...
- BE는 엑셀 해석·매핑 후 `/ai/categories/product-index/append`로 상품 목록을 전달합니다.
  추가 업로드 경로의 상품별 DB 피드백 조회·저장 및 사용자별 통계 갱신은 수행하지 않습니다.
- 신규·갱신 수는 공유 인덱스의 정규화 상품명 기준입니다. 요청 내 중복은 마지막 값으로 합칩니다.
- 기존 상품의 임베딩 입력이 바뀌면 해당 상품만 재임베딩합니다. 나머지 벡터는 재사용합니다.
  변경 벡터가 있으면 FAISS 인덱스 저장 구조를 다시 구성하므로 메모리·저장 비용은 발생합니다.
- `/ai/categories/product-index/stats`는 공유 인덱스에서 집계하며 결과를 메모리에 캐시합니다.
  인덱스 저장 후 무효화하며, 총 상품 수는 고유 상품 수이고 다중 카테고리는 각 카테고리에 포함합니다.
- 기존 DB 테이블·이력은 삭제하지 않습니다. 단건 피드백 API의 DB 기록은 별도로 유지합니다.
- 여전히 동기 HTTP 요청입니다. 장시간 요청 끊김을 해결하는 비동기 작업 전환은 별도 과제입니다.
- 기존에 이미 이름과 벡터가 불일치하는 데이터는 자동 복구되지 않습니다.

### 선택적 이미지 분석 (실험 기능)

`STOREPILOT_IMAGE_ANALYSIS_ENABLED=true`로 켜면 엑셀의 `목록이미지1` URL을 이용합니다.
Alias 또는 상품 근거 자동 선택 조건을 만족한 상품은 이미지 API를 호출하지 않습니다.
후보는 있지만 LLM 판단이 필요한 상품만 OpenAI 이미지 분석을 수행하며, 분석 결과를
카테고리 선택과 BE 키워드 생성에 함께 사용합니다. 기존 임베딩 재생성은 필요 없습니다.

- 기본 모델: `STOREPILOT_IMAGE_ANALYSIS_MODEL=gpt-4.1-mini`, 이미지 상세도 `low`
- 키: `STOREPILOT_IMAGE_ANALYSIS_API_KEY` 또는 미설정 시 기존 `STOREPILOT_LLM_API_KEY`
- OpenAI API 전용이며 다른 공급자의 LLM 키는 사용할 수 없습니다.
- StorePilot 서버는 이미지 URL에 직접 접속하지 않습니다. URL을 OpenAI에 전달하여
  OpenAI가 이미지를 가져옵니다. 따라서 이미지 URL과 상품명이 외부 서비스에 전달됩니다.
- 분석 동시 실행은 프로세스 전체 최대 4개, 개별 호출 타임아웃 15초,
  카테고리 배치당 분석 시작 예산 60초입니다. 예산을 넘은 상품은 텍스트로 처리합니다.
- 429 발생 시 프로세스 내 이미지 호출을 60초 동안 중단합니다. 기존 카테고리 판단 API의
  별도 429 문제까지 해결하는 기능은 아닙니다.
- URL 누락·이미지 접근 오류·분석 실패·상품명 불일치 시 기존 텍스트 처리로 진행합니다.
- `imageAnalysis`와 `imageAnalysisStatus`를 AI 응답에 포함하고,
  `image_analysis_usage` / `image_analysis_result` 로그에 토큰 사용량·상태·시간을 기록합니다.
- 이미지 원본 및 분석 결과의 영속 캐시는 아직 없습니다. 재작업 시 다시 호출될 수 있습니다.
- 신뢰도는 참고용이며 임계값으로 자동 채택하지 않습니다. BE 키워드에는 상품 종류·색상·형태만
  사용하고, 이미지 유래 키워드에 `이미지 분석 보조 정보` 근거를 붙입니다.

기본값은 비활성화입니다. 먼저 소량 파일로 정확도와 비용을 비교한 뒤 활성화하세요.
